...
public class OrderServiceImpl implements OrderService {
    ...
    for (OrderItemRequestDto itemRequest : orderRequest.getItems()) {
      Product product =
          productRepository
              ...
              .orElseThrow(
                  () -> new ResourceNotFoundException("Product", itemRequest.getProductId()));
  ...
  public List<OrderResponseDto> getMyOrders() {
    Long currentUserId = SecurityUtils.getCurrentUserId();
    List<Order> orders = orderRepository.findByUserIdWithItems(currentUserId);
  ...
  public OrderResponseDto updateOrderStatus(
      Long id, OrderStatusUpdateRequestDto statusUpdateRequest) {
    Order order =
